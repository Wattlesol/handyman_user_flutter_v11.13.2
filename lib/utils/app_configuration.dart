...
Future<void> setAppConfigurations(AppConfigurationModel data) async {
  ...
  await appConfigurationStore.setCurrencyCode(
      data.currencyCode.validate(value: DEFAULT_CURRENCY_CODE));
  await appConfigurationStore.setCurrencyPosition(
      data.currencyPosition.validate(value: CURRENCY_POSITION_LEFT));
  await appConfigurationStore.setCurrencySymbol(
      data.currencySymbol.validate(value: DEFAULT_CURRENCY_SYMBOL));
  ...
  appConfigurationStore.setCancellationChargeAmount(
      double.parse(data.cancellationChargeAmount?.toString() ?? '0')
          .validate());
  appConfigurationStore
  ...
  appConfigurationStore.setISUserAuthorized(data.isUserAuthorized ?? true);
...
class AppConfigurationModel {
  ...
  AppConfigurationModel.fromJsonMap(Map<String, dynamic> map)
      : siteName = map["site_name"],
        ...
        advancePaymentStatus = map["advance_payment_status"] ?? 0,
        slotServiceStatus = map["slot_service_status"] ?? 0,
        digitalServiceStatus = map["digital_service_status"] ?? 0,
        servicePackageStatus = map["service_package_status"] ?? 0,
        serviceAddonStatus = map["service_addon_status"] ?? 0,
        jobRequestServiceStatus = map["job_request_service_status"] ?? 0,
        socialLoginStatus = map["social_login_status"] ?? 0,
        googleLoginStatus = map["google_login_status"] ?? 0,
        appleLoginStatus = map["apple_login_status"] ?? 0,
        otpLoginStatus = map["otp_login_status"] ?? 0,
        cancellationCharge = map["cancellation_charge"] ?? 0,
        cancellationChargeAmount = map["cancellation_charge_amount"],
        cancellationChargeHours = map["cancellation_charge_hours"] ?? 0,
        onlinePaymentStatus = map["online_payment_status"] ?? 0,
        blogStatus = map["blog_status"] ?? 0,
        maintenanceMode = map["maintenance_mode"] ?? 0,
        walletStatus = map["wallet_status"] ?? 0,
        chatGptStatus = map["chat_gpt_status"] ?? 0,
        testChatGptWithoutKey = map["test_chat_gpt_without_key"] ?? 0,
