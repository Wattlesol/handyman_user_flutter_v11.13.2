...
//region Store & Product APIs
Future<StoreResponse> getStore() async {
  try {
    var response = await handleResponse(
        await buildHttpResponse('store', method: HttpMethodType.GET));

    // Handle case where API returns error or unexpected format
    if (response is Map &&
        response.containsKey('message') &&
        response['message'] == 'Failed') {
      // Return empty response instead of throwing error
      appStore.setLoading(false);
      return StoreResponse(store: null, featuredProducts: [], categories: []);
    }

    var res = StoreResponse.fromJson(response);
    appStore.setLoading(false);
    return res;
  } catch (e) {
    appStore.setLoading(false);
    // Return empty response instead of throwing error for better UX
...
Future<ProductResponse> getProducts({
  ...
  try {
    ...
    var res = ProductResponse.fromJson(response);
    appStore.setLoading(false);
    return res;
  } catch (e, stackTrace) {
    appStore.setLoading(false);
...
Future<ProductResponse> getFeaturedProducts({int? perPage = 10}) async {
  try {
    var response = await handleResponse(await buildHttpResponse(
        'featured-products?per_page=$perPage',
        method: HttpMethodType.GET));

    // Handle case where API returns {"message":"Failed"}
    if (response is Map &&
        response.containsKey('message') &&
        response['message'] == 'Failed') {
      // Return empty response instead of throwing error
    ...
    // Debug: Print response structure
    print('Featured products response type: ${response.runtimeType}');
    if (response is Map) {
      print('Featured products response keys: ${response.keys}');
    }
...
Future<ProductData> getProductDetails(int productId) async {
  try {
    var response = await handleResponse(await buildHttpResponse(
        'products/$productId',
        method: HttpMethodType.GET));
...
Future<ProductCategoryResponse> getProductCategories() async {
  try {
    var res = ProductCategoryResponse.fromJson(await handleResponse(
        await buildHttpResponse('product-categories',
            method: HttpMethodType.GET)));
    appStore.setLoading(false);
    return res;
  } catch (e) {
...
Future<ProductResponse> searchProducts({
  ...
  try {
    ...
    // Debug: Print response structure
    print('Search products response type: ${response.runtimeType}');
    if (response is Map) {
      print('Search products response keys: ${response.keys}');
    }
...
Future<Map<String, dynamic>> createOrder({
  ...
  try {
    ...
    if (variantId != null) request['variant_id'] = variantId;
    if (deliveryNotes != null && deliveryNotes.isNotEmpty)
      request['delivery_notes'] = deliveryNotes;

    var response = await handleResponse(await buildHttpResponse('orders',
        request: request, method: HttpMethodType.POST));
...
Future<Map<String, dynamic>> getMyOrders({
  ...
  try {
    String queryParams = '?per_page=$perPage';
    ...
    if (response is Map) {
      print('Orders response keys: ${response.keys}');
      if (response['data'] != null) {
        print('Orders data type: ${response['data'].runtimeType}');
        if (response['data'] is Map && response['data']['data'] != null) {
          print(
              'Orders nested data type: ${response['data']['data'].runtimeType}');
          if (response['data']['data'] is List) {
            var dataList = response['data']['data'] as List;
            print('Orders list length: ${dataList.length}');
            if (dataList.isNotEmpty) {
              print('First order item type: ${dataList[0].runtimeType}');
            }
          }
        } else if (response['data'] is List) {
          var dataList = response['data'] as List;
          print('Direct orders list length: ${dataList.length}');
          if (dataList.isNotEmpty) {
            print('First order item type: ${dataList[0].runtimeType}');
          }
        }
...
Future<Map<String, dynamic>> getOrderDetails(int orderId) async {
  try {
    var response = await handleResponse(
        await buildHttpResponse('orders/$orderId', method: HttpMethodType.GET));
...
Future<Map<String, dynamic>> cancelOrder(int orderId, String reason) async {
  try {
    Map<String, dynamic> request = {'reason': reason};
